...
def prepare_dataset(output_path, input_folder, rpg_file, label_names=['CODE_CULT']):
    """
...
def crop_and_write(output_path, tifs, date_index, polygons, lab_rpg):
    ...
    abels = dict([(l, {}) for l in lab_rpg.keys()])
    for tifname in tifs:
        ...
        with rasterio.open(tifname) as src:
            for parcel_id, p in tqdm(polygons.items()):
                try:
                    # crop parcel
                    try:
                        crop, transform = rm.mask(src, [p], crop=True, nodata=np.iinfo(np.uint16).max)
                    except ValueError:
                    ...
                    pixels = pixels.reshape((1, *pixels.shape)) # (1, C, )
...
def parse_rpg(rpg_file, label_names=['CODE_CULT']):
...
if __name__ == '__main__':
    rpg_file = 'OCS_PRED.geojson'
    input_folder = 'data_preparation'
    out_path = './OCS_PRED_CULT'

    prepare_dataset(out_path, input_folder, rpg_file, label_names=['CODE_CULT'])
