...
import argparse
import pprint
from models.stclassifier import PseTae, PseLTae, PseGru, PseTempCNN,PseLTae_pretrained
from dataset import PixelSetData, PixelSetData_preloaded
from learning.focal_loss import FocalLoss
from learning.weight_init import weight_init
from learning.mestrics import mIou, confusion_matrix_analysis
...
def get_config_file(model_path, model_name):
    return os.path.join(model_path, model_name+"conf_mat.pkl" )

def get_model_file(model_path, model_name):
    return os.path.join(model_path, model_name + "model.pth.tar")

def get_result_file(model_path, model_name):
    return os.path.join(model_path, model_name+"test_metrics.json")
...
def main(config):
 ...
 #*********************************************************************************************************  
        get_model_file('/config_fig2.json','' )
        get_result_file('PSE-LTAE-pre-trained-weights/Fold_1','')
        get_config_file('PSE-LTAE-pre-trained-weights/Fold_1','')
        model=load_model('PSE-LTAE-pre-trained-weights/Fold_1','' )
        save_model(model, 'PSE-LTAE-pre-trained-weights/Fold_1', '')
        #*****************************************************************************************************************
        ...
        # freeze backbone layers
            for param in model.children():
              count +=1
              if count < 1: #freezing first1 layers
...
if __name__ == '__main__':
    ...
    parser.add_argument('--res_dir', default='./Sensibilite/num_workers_results1', help='Path to the folder where the results should be stored')
    ...
    parser.add_argument('--npixel', default=32, type=int, help='Number of pixels to sample from the input images')
