...
def transfer ( device='cuda', fold='all'):
  ...
  weight_folder = 'PSE-LTAE-pre-trained-weights' 
  hyperparameters = model_config

  fold_folders = [os.path.join(weight_folder, f) for f in os.listdir(weight_folder) if
                             os.path.isdir(os.path.join(weight_folder, f))]
  if fold == 'all':
    n_folds = len(fold_folders)
  else:
    n_folds = 1
    fold_folders = [fold_folders[int(fold) - 1]]
    

  print('Loading pre-trained models . . .')
  for f in fold_folders:
    m = PseLTae(**model_config)
...
def main(config):
    ...
    for fold, (train_loader, val_loader, test_loader) in enumerate(loaders):
        ...
        d=transfer ( device='cuda', fold='all')
        mod.load_state_dict(d['state_dict'])
        model_instances.append(mod)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', default=502, type=int, help='Batch size')
